...
const makePage1Recipes = (): RecipeWithAuthor[] =>
  Array.from({ length: 12 }, (_, i) => ({
    ...
    description:
      i === 0
        ? "A classic Italian pasta dish with eggs and pancetta."
        : `Description ${i + 1}.`,
    recipe:
      i === 0
        ? "Boil pasta. Fry pancetta. Mix eggs and cheese. Combine."
        : "Step 1. Step 2. Serve.",
    authorId: "user1",
    authorUsername: "chef",
    createdAt: Date.now(),
...
const makePage2Recipes = (): RecipeWithAuthor[] =>
  Array.from({ length: 12 }, (_, i) => ({
    slug: `recipe-p2-${i + 1}`,
    id: 100 + i + 1,
    title: `Recipe P2-${i + 1}`,
...
/** The user ID used in mock mode — matches recipe authorId so Edit/Delete render. */
export const MOCK_MODE_USER_ID = "user1";

/** Profile returned in mock mode — id matches authorId on all recipes. */
export const MOCK_MODE_PROFILE: UserProfile = {
  id: MOCK_MODE_USER_ID,
  username: "chef",
