...
test.describe("Recipes list page", () => {
	/**
	 * Tests that don't depend on auth state — they use the default anonymous
	 * path since no localStorage tokens are set.
	 */
	test.describe("UI behavior", () => {
		...
		test("typing 3+ characters updates the URL query param", async ({
			...
			await page.waitForURL(/q=pasta/, { timeout: 15000 });
		...
		test("pressing Enter submits the search", async ({ page }) => {
			...
			await page.waitForURL(/q=soup/, { timeout: 15000 });
	...
	/**
	 * When no auth tokens are present in localStorage (or the refresh token is
	 * expired), the recipes page must call the anonymous /anonymous/recipes
	 * endpoint instead of the authenticated /recipes endpoint.
	 */
	test.describe("anonymous (not logged in)", () => {
		...
		test("renders recipes from the anonymous endpoint", async ({ page }) => {
			await page.goto("/recipes");
	...
	/**
	 * When the user is logged in (valid refreshExpiresAt in localStorage),
	 * the recipes page must call the authenticated /recipes endpoint instead
	 * of the anonymous /anonymous/recipes endpoint.
	 */
	test.describe("authenticated (logged in)", () => {
		...
		test("fetches recipes from the authenticated /recipes endpoint", async ({
			page,
		}) => {
			const requestPromise = page.waitForRequest(
				(request) =>
		...
		test("renders recipes when fetched from the authenticated endpoint", async ({
			page,
		}) => {
			await page.goto("/recipes");
			await expect(page.getByText("Pasta Carbonara")).toBeVisible({
